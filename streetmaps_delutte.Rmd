...
```{r}
getbb("De Lutte The Netherlands")
```
...
```{r}
streets <- getbb("De Lutte The Netherlands") %>%
  opq()%>%
...
```{r}
small_streets <- getbb("De Lutte The Netherlands") %>%
  opq()%>%
  add_osm_feature(key = "highway",
...
ggplot() +
  geom_sf(data = streets$osm_lines,
          ...
          size = .4,
          alpha = .8) +
  coord_sf(xlim = c(6.934325, 7.057227),
           ylim = c(52.279489, 52.349259),
           expand = FALSE)
```
...
theme_void() +
  ...
  ggsave("delutte_bw.tiff", height=6, width=6, units='in', dpi=600)
...
ggplot() +
  ...
  ggsave("delutte_dark.tiff", height=6, width=6, units='in', dpi=600)
  #tiff("test.tiff", units="in", width=6, height=6, res=600, compression = "lzw")
